...
class MSOEvaluation:
    ...
    def witness_distance(self, i, P, Q):
        ...
        if not self.use_double_proj:
            ...
            dist = (C_pq[i_p, :].min(1)[0].sqrt() + C_pq[:, i_q].min(0)[0].sqrt()) / 2
    ...
    def eval(self):
        ...
        for i in pbar:
            ...
            if isinstance(d_witness, float):
                ...
                precision = [
                    (1.0 if d_witness == 0 else 0.0)
                    for threshold in precision_thresholds
                ]
            else:
                divergence = d_witness.mean()
                precision = [
                    (d_witness < threshold).sum() / d_witness.shape[0]
                    for threshold in precision_thresholds
                ]
        ...
        for key in first_result:
            if key == "precision":
                self.avg_results[key] = torch.stack(
                    [r[key] for r in self.eval_results], -1
                ).mean(-1)
            else:
                self.avg_results[key] = torch.tensor(
                    [r[key] for r in self.eval_results], dtype=torch.float32
                ).mean()
