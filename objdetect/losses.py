...
class ClassificationLoss(nn.Module):
    ...
    def forward(self, batched_inputs: List[Dict[str, torch.Tensor | Instances]]):
        ...
        box_distances = self.box_distances(pred_boxes, gt_padded)  # N x A x B (A is # pred_boxes, B is max # gt_boxes)
        box_distances_masked = torch.where(
            gt_masks.unsqueeze(-2).expand(-1, box_distances.shape[1], -1),  # N x B -> N x A x B
            box_distances,
            torch.full_like(box_distances, 1e8),
        )

        closest_gt_distance, closest_gt_boxes = box_distances_masked.min(-1) # both N x A
        
        target_gt_classes = []
        gt_is_not_empty_mask = []
        for i, bi in enumerate(batched_inputs):
            if bi["instances"]._fields["gt_classes"].shape[0] > 0:
                target_gt_classes.append(bi["instances"]._fields["gt_classes"][closest_gt_boxes[i]])
                gt_is_not_empty_mask.append(1)
            else:
                target_gt_classes.append(torch.full_like(closest_gt_boxes[i], 0)) # TODO: Oof, needa handle the case where there are no gt_classes
                gt_is_not_empty_mask.append(0)

        target_gt_classes = torch.stack(target_gt_classes)

        class_logits = []
        for bi in batched_inputs:
            class_logits.append(bi["class_logits"]) # appending AxC
        class_logits = torch.stack(class_logits) # NxAxC
        N, A, C = class_logits.shape

        target_onehot = torch.nn.functional.one_hot(target_gt_classes, num_classes=class_logits.shape[2]).float() # NxAxC
        ...
        gt_is_not_empty_mask = torch.tensor(gt_is_not_empty_mask, dtype=bool, device=device).repeat(1, A) # shape NxA
        loss = torch.where(
            gt_is_not_empty_mask,
            loss,
