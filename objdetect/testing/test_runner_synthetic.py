...
def synthetic_train_function():
    with open(
        f"/mnt/tcqian/danielxu/gdp-objdetect/objdetect/datasets/synthetic_dataset/annotations/instances_train.json",
        "r",
    ) as fp:
...
dataloader = build_detection_train_loader(
    dataset, mapper=DatasetMapper(cfg, is_train=True), total_batch_size=8, num_workers=1
)  # we have to figure out what mapper is bruh...
